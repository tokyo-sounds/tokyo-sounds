...
function FloatingNavbar({ onStartClick }: { onStartClick: () => void }) {
        ...
        <div className="relative z-10 flex items-center gap-1 px-2">
          {navLinks.map((link) => (
            <motion.a 
              key={link.href}
              href={link.href}
              className="relative px-5 py-2.5 text-sm font-medium text-gray-600 rounded-xl transition-colors duration-300"
...
function Footer() {
  ...
  return (
    <footer className="relative py-8 px-6 bg-gray-900 text-white">
  ...
  return (
    <>
      ...
      <div className="hidden md:block w-full bg-gray-100" style={{ touchAction: 'pan-y' }}>
        <FloatingNavbar onStartClick={() => setIsPreflightOpen(true)} />
