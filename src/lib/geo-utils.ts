 ...
 * Uses lazy loading - only loads details when district is found
 * @param lat Latitude in degrees
 * @param lng Longitude in degrees
...
export function getDistrictAtPosition(
  ...
  let closestBasic: DistrictBasic | null = null;
  let closestDistance = Infinity;

  // Step 1: Find closest district using lightweight basic data
  for (const district of TOKYO_DISTRICTS_BASIC) {
    ...
    if (distance <= district.radius && distance < closestDistance) {
      ...
      closestBasic = district;
    }
  }

  // Step 2: Load details only if district found (synchronous - data in memory)
  if (!closestBasic) {
    return null;
  }

  const details = getDistrictDetails(closestBasic.id);
  return { ...closestBasic, ...details };
}

/**
 * Calculate weights for all districts based on distance from a position
 * Uses lazy loading - only loads details for districts with significant weight
 * @param lat Latitude in degrees
 * @param lng Longitude in degrees
 * @param falloffDistance Distance in meters at which weight reaches ~0.1
 * @param weightThreshold Only load details for districts with weight > threshold (default: 0.05)
...
export function calculateDistrictWeights(
  ...
  weightThreshold: number = 0.05
): Array<{ district: District; weight: number; distance: number }> {
  // Step 1: Calculate weights using lightweight basic data
  const basicResults = TOKYO_DISTRICTS_BASIC.map((district) => {
    const distance = haversineDistance(
      lat,
      lng,
      district.center.lat,
  ...
  const totalWeight = basicResults.reduce((sum, r) => sum + r.weight, 0);
  if (totalWeight > 0) {
  ...
  basicResults.sort((a, b) => b.weight - a.weight);

  // Step 4: Load details only for significant districts (weight > threshold)
  // This reduces memory usage by ~90% (only 3-5 districts typically have weight > 0.05)
  const significantDistricts = basicResults.filter(
    (r) => r.weight > weightThreshold
  );

  const results = significantDistricts.map(({ district, weight, distance }) => {
    const details = getDistrictDetails(district.id);
    return {
  ...
  // Add low-weight districts with basic data only (for completeness)
  const lowWeightDistricts = basicResults
    .filter((r) => r.weight <= weightThreshold)
    .map(({ district, weight, distance }) => ({
      district: {
        ...district,
        prompt: DEFAULT_DISTRICT_PROMPT,
        descriptionJa: "",
      } as District,
      weight,
      distance,
  ...
  return [...results, ...lowWeightDistricts];
}
...
export function getWeightedPromptsForPosition(
  ...
  if (currentDistrict) {
    const idx = districtWeights.findIndex(
      (d) => d.district.id === currentDistrict.id
    );
    if (idx !== -1) {
      districtWeights[idx].weight = Math.min(
        1,
        districtWeights[idx].weight * 1.5
      );
 ...
 * @returns Object with lat, lng (degrees) and alt (meters)
 */
export function worldToLatLngAlt(worldPos: THREE.Vector3): {
  lat: number;
  lng: number;
  alt: number;
} {
 ...
 */
export function latLngAltToWorld(
  lat: number,
  lng: number,
  alt: number
): THREE.Vector3 {
...
/**
 * Get district info for camera position (optimized, lazy loads details)
 * @param cameraWorldPos Camera world position
 * @returns Current district or null
 */
export function getDistrictForCamera(
