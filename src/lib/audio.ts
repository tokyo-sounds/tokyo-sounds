...
export interface AudioSession {
  ...
  updateParam(
    ...
    param: string,
    value: any,
    opts?: { atTime?: number; record?: boolean }
  ): void;
...
function validateParam(nodeType: NodeType, param: string, value: any): any {
  ...
  if (constraint.type === "number") {
    ...
    if (constraint.min !== undefined && numVal < constraint.min) {
      if (DEBUG_AUDIO)
        console.warn(
          `Clamping ${param} from ${numVal} to min ${constraint.min}`
        );
    ...
    if (constraint.max !== undefined && numVal > constraint.max) {
      if (DEBUG_AUDIO)
        console.warn(
          `Clamping ${param} from ${numVal} to max ${constraint.max}`
        );
  ...
  if (constraint.type === "enum") {
    if (!constraint.enum?.includes(value)) {
      throw new Error(
        `Invalid enum value ${value} for ${param}. Must be one of: ${constraint.enum?.join(
          ", "
        )}`
      );
    }
  ...
  for (const nodeDef of spec.nodes) {
    ...
    if (nodeType === "Player") {
      ...
      if (audioBuffer) {
        ...
        if (DEBUG_AUDIO) {
          console.log(
            `Created Player ${nodeDef.id} with pre-loaded buffer (${audioBuffer.duration}s)`
          );
        }
      } else {
        instance = new Tone.Player();
        console.warn(
          `No pre-loaded buffer found for player ${nodeDef.id} (assetId: ${assetId})`
        );
      }
  ...
  for (const conn of spec.connections) {
    ...
    if (fromNode && toNode) {
      try {
        ...
        if (DEBUG_AUDIO)
          console.warn(
            `Failed to connect ${conn.from.id} -> ${conn.to.id}:`,
            err
          );
      }
    }
  ...
  for (const [id, node] of registry.entries()) {
    ...
    if (!hasOutgoing) {
      try {
        ...
        if (DEBUG_AUDIO)
          console.warn(`Failed to connect ${id} to export bus:`, err);
      }
    }
  }
      ...
      if (assetDef) {
        ...
        if (predecoded) {
          ...
          try {
            ...
            if (DEBUG_AUDIO) {
              console.log(
                `Player ${
                  nodeDef.id
                } loaded from pre-decoded buffer (${predecoded.duration.toFixed(
                  2
                )}s)`
              );
            }
            ...
            console.error(
              `Failed to init player ${nodeDef.id} from predecoded buffer:`,
              err
            );
          }
        } else {
          ...
          try {
            ...
            if (DEBUG_AUDIO) {
              console.log(
                `Player ${nodeDef.id} loaded successfully (direct URL)`
              );
            }
          } catch (err) {
...
function scheduleAutomations(
  registry: Map<string, RuntimeNode>,
  spec: GraphSpec,
  context: BaseAudioContext
): void {
...
async function encodeWav(
  buffer: AudioBuffer,
  opts: CommitOptions = {}
): Promise<{ blob: Blob; url: string; audioHash: string }> {
  ...
  if (peak > 1.0) {
    ...
    if (DEBUG_AUDIO) {
      console.log(
        `Scaling down to prevent clipping: peak=${peak.toFixed(
          3
    ...
    if (DEBUG_AUDIO) {
      console.log(
        `Normalizing loud signal: peak=${peak.toFixed(
          3
        )} → scale=${scale.toFixed(3)}`
      );
    }
  ...
  if (DEBUG_AUDIO) {
    console.log(
      `WAV encoding: peak=${peak.toFixed(3)}, scale=${scale.toFixed(
        3
      )}, DC offsets=${dcOffsets.map((v) => v.toFixed(6)).join(", ")}`
    );
  }
...
class AudioSessionImpl implements AudioSession {
  ...
  private eventHandlers: Map<AudioSessionEvent, Set<(...args: any[]) => void>> =
    new Map();
  ...
  private bufferCache: Map<
    string,
    { buffer: AudioBuffer; timestamp: number; size: number }
  > = new Map();
  ...
  updateParam(
    ...
    if (DEBUG_AUDIO) {
      console.log(
        `[lib/audio.ts] Param ${nodeId}.${param} = ${validValue} (cache invalidated)`
      );
    }
  }
  ...
  automate(nodeId: string, param: string, def: Partial<AutomationDef>): string {
    const id =
      def.id || `auto_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    ...
    if (DEBUG_AUDIO) {
      console.log(
        `[lib/audio.ts] Automation added: ${id} for ${nodeId}.${param}`
      );
    }
  ...
  private filterSpecForChain(spec: GraphSpec, chain: Set<string>): GraphSpec {
    ...
    const filteredAssets = spec.assets.filter((asset) =>
  ...
  async commit(opts: CommitOptions = {}): Promise<RenderResult> {
    ...
    if (opts.nodeId) {
      ...
      if (DEBUG_AUDIO) {
        console.log(
          `[commit] Rendering chain for node ${opts.nodeId}:`,
          Array.from(chain)
        );
      }
    }
    ...
    for (const asset of spec.assets) {
      try {
        ...
        if (cached) {
          ...
          if (DEBUG_AUDIO) {
            console.log(
              `[commit] Reusing cached buffer ${asset.id} (${
                audioBuffer.duration
              }s, ${(cached.size / (1024 * 1024)).toFixed(2)}MB)`
            );
          }
        } else {
          ...
          if (!response.ok) {
            console.error(
              `Failed to fetch asset ${asset.id}: ${response.status} ${response.statusText}`
            );
            continue;
          }

          const arrayBuffer = await response.arrayBuffer();
          audioBuffer = await this.context.decodeAudioData(
            arrayBuffer.slice(0)
          );
        }
        ...
        if (decodedSizeMB > maxBufferSizeMB) {
          console.warn(
            `Asset ${
              asset.id
            } exceeds max buffer size after decoding (${decodedSizeMB.toFixed(
              1
            )}MB > ${maxBufferSizeMB}MB), skipping`
          );
          continue;
        }
        ...
        if (!cached) {
          ...
          if (decodedBytes > this.maxCacheSize) {
            console.warn(
              `Asset ${asset.id} (${decodedSizeMB.toFixed(
                1
              )}MB) is larger than max cache size (${(
                this.maxCacheSize /
                (1024 * 1024)
              ).toFixed(1)}MB), skipping`
            );
            continue;
          }

          if (totalCacheSize + decodedBytes > this.maxCacheSize) {
            const sortedCache = Array.from(this.bufferCache.entries()).sort(
              (a, b) => a[1].timestamp - b[1].timestamp
            );

            while (
              totalCacheSize + decodedBytes > this.maxCacheSize &&
              ...
              if (DEBUG_AUDIO) {
                console.log(
                  `[commit] Evicted old buffer ${key} to free ${(
                    oldest.size /
                    (1024 * 1024)
                  ).toFixed(2)}MB`
                );
          ...
          if (DEBUG_AUDIO) {
            console.log(
              `[commit] Cached asset ${asset.id} (${audioBuffer.duration}s, ${
                audioBuffer.numberOfChannels
              }ch, ${(decodedBytes / (1024 * 1024)).toFixed(2)}MB)`
            );
          }
        }
    ...
    if (DEBUG_AUDIO) {
      console.log(
        `Offline render: ${duration}s at ${sampleRate}Hz (context: ${contextRate}Hz)`
      );
    ...
    if (estimatedSamples > maxSamples) {
      ...
      throw new Error(
        `Render duration too long: ${duration.toFixed(1)}s would use ~${(
          (estimatedSamples * 4) /
          (1024 * 1024)
        ).toFixed(0)}MB. ` +
          `Maximum duration at ${sampleRate}Hz is ${maxDuration.toFixed(
            1
          )}s. ` +
    ...
    const buffer = await Tone.Offline(
      async (offlineContext) => {
        const registry = await buildRuntimeGraphWithBuffers(
          spec,
          offlineContext.rawContext,
          preloadedBuffers
        );
        ...
        for (const [_, node] of registry.entries()) {
          if (node.def.type === "Player" && node.instance.start) {
            ...
            const hasValidBuffer =
              buffer && buffer.length > 0 && buffer.duration > 0;

            if (hasValidBuffer) {
              node.instance.start(0);
              if (DEBUG_AUDIO) {
                console.log(
                  `Started player ${node.def.id} in offline context (buffer duration: ${buffer.duration}s, samples: ${buffer.length})`
                );
              }
            } else {
              console.error(
                `Player ${node.def.id} buffer invalid - length=${buffer?.length}, duration=${buffer?.duration}`
              );
            }
          }
        }
    ...
    const { blob, url, audioHash } = await encodeWav(
      buffer.get() as AudioBuffer,
      opts
    );
  ...
  bindSpatial(
    ...
    const rolloffFactor =
      opts.rolloffFactor !== undefined ? opts.rolloffFactor : 1;
    const maxDistance =
      opts.maxDistance !== undefined ? opts.maxDistance : 10000;
    ...
    const enableCulling =
      opts.enableCulling !== undefined ? opts.enableCulling : true;
    const cullDistance =
      opts.cullDistance !== undefined ? opts.cullDistance : 500;
    const resumeDistance =
      opts.resumeDistance !== undefined
        ? opts.resumeDistance
        : cullDistance * 0.8;
    const fadeMs =
      opts.cullFadeMs !== undefined ? Math.max(0, opts.cullFadeMs) : 30;
    const enableLevelMeter =
      opts.enableLevelMeter !== undefined ? opts.enableLevelMeter : false;
    ...
    if (mode === "live") {
      ...
      if (toneContext === threeContext) {
        try {
          ...
          if (DEBUG_AUDIO) {
            console.log(
              `[lib/audio.ts/bindSpatial] Live mode: ${nodeId} connected directly (same context)`
            );
          }
        } catch (err) {
          console.error(
            `[lib/audio.ts/bindSpatial] Failed to connect directly for ${nodeId}:`,
            err
          );
          throw new Error(
            `Failed to create spatial audio: ${
        ...
        if (DEBUG_AUDIO) {
          console.log(
            `[lib/audio.ts/bindSpatial] Context mismatch detected. Using MediaStream bridge to connect Tone.js (${toneContext.sampleRate}Hz) to THREE.js (${threeContext.sampleRate}Hz).`
          );
        }
        ...
        try {
          if (!toneContext || !threeContext) {
            throw new Error(
              "[lib/audio.ts/bindSpatial] AudioContext not available for MediaStream bridge"
            );
          }

          if (
            toneContext instanceof OfflineAudioContext ||
            threeContext instanceof OfflineAudioContext
          ) {
            throw new Error(
              "[lib/audio.ts/bindSpatial] MediaStream bridge requires AudioContext, not OfflineAudioContext"
            );
          }

          const destination = (
            toneContext as AudioContext
          ).createMediaStreamDestination();
          if (!destination || !destination.stream) {
            throw new Error(
              "[lib/audio.ts/bindSpatial] Failed to create MediaStreamDestination"
            );
          }
          ...
          mediaStreamSource = (
            threeContext as AudioContext
          ).createMediaStreamSource(destination.stream);
          if (!mediaStreamSource) {
            throw new Error(
              "[lib/audio.ts/bindSpatial] Failed to create MediaStreamSource"
            );
          }
          ...
          if (DEBUG_AUDIO) {
            console.log(
              `[lib/audio.ts/bindSpatial] Live mode: ${nodeId} connected via MediaStream bridge`,
              {
          ...
          console.error(
            `[lib/audio.ts/bindSpatial] Failed to create MediaStream bridge for ${nodeId}:`,
            err
          );
          throw new Error(
            `Failed to create spatial audio bridge: ${
              err instanceof Error ? err.message : "Unknown error"
            }`
          );
        }
      ...
      if (DEBUG_AUDIO) {
        console.log(
          `[lib/audio.ts/bindSpatial] Committed mode: ${nodeId} awaiting buffer`
        );
      }
    }
    ...
    if (opts.enableLevelMeter) {
      try {
        ...
        if (DEBUG_AUDIO) {
          console.log(
            `[lib/audio.ts/bindSpatial] Created AnalyserNode for ${nodeId}`
          );
        }
      } catch (err) {
        if (DEBUG_AUDIO)
          console.warn(
            `[lib/audio.ts/bindSpatial] Failed to create AnalyserNode:`,
            err
          );
      }
    }
    ...
    const disposer = () => {
      if (mediaStreamSource) {
        try {
          ...
          if (DEBUG_AUDIO)
            console.warn(
              `[lib/audio.ts/bindSpatial] Error disconnecting MediaStream bridge:`,
              err
            );
        }
      }
      ...
      if (analyser) {
        try {
          ...
          if (DEBUG_AUDIO)
            console.warn(
              `[lib/audio.ts/bindSpatial] Error disconnecting AnalyserNode:`,
              err
            );
        }
      }
      ...
      try {
        ...
        if (DEBUG_AUDIO)
          console.warn(
            `[lib/audio.ts/bindSpatial] Error disposing PositionalAudio (harmless):`,
            err
          );
      }
      ...
      if (DEBUG_AUDIO) {
        console.log(
          `[lib/audio.ts/bindSpatial] Disposed binding for ${nodeId}`
        );
      }
    };
  ...
  private disposeNodeChain(nodeId: string): void {
    ...
    try {
      ...
      if (DEBUG_AUDIO)
        console.warn(
          `[Freeze] Failed to disconnect exportBus for ${nodeId}:`,
          err
        );
    }
  }
  ...
  freezeSpatialBinding(nodeId: string, result: RenderResult): void {
    ...
    if (binding.mode === "live") {
      try {
        ...
        if (DEBUG_AUDIO)
          console.log(
            `[lib/audio.ts/freezeSpatialBinding] Disconnect failed:`,
            err
          );
      }
    ...
    if (DEBUG_AUDIO) {
      console.log(
        `[Freeze] ✅ ${nodeId} switched to COMMITTED mode (${result.duration.toFixed(
          2
        )}s buffer)`
      );
      console.log(
        `[Freeze] Culling config: cullDist=${binding.cullDistance}, resumeDist=${binding.resumeDistance}, enabled=${binding.enableCulling}`
      );
    }
  ...
  unfreezeSpatialBinding(nodeId: string): void {
    ...
    try {
      ...
      if (
        ...
        throw new Error(
          "MediaStream bridge requires AudioContext, not OfflineAudioContext"
        );
      }
      ...
      if (DEBUG_AUDIO) {
        console.log(
          `[lib/audio.ts/unfreezeSpatialBinding] ${nodeId} switched back to live mode with MediaStream bridge`
        );
      }
    } catch (err) {
      console.error(
        `[lib/audio.ts/unfreezeSpatialBinding] Failed to recreate MediaStream bridge for ${nodeId}:`,
        err
      );
      throw err;
    }
  ...
  setPreview(nodeId: string, enabled: boolean): void {
    ...
    if (enabled && !wasEnabled) {
      ...
      try {
        ...
        if (DEBUG_AUDIO)
          console.warn(`[lib/audio.ts/setPreview] Disconnect warning:`, err);
      }
  ...
  updateSpatialCulling(cameraPosition: THREE.Vector3): void {
    ...
    for (const [nodeId, binding] of this.spatialBindings) {
      ...
      if (binding.mode === "live") {
        binding.lastDistance = cameraPosition.distanceTo(
          binding.object3D.position
        );
        continue;
      }
      ...
      if (!binding.isCulled && shouldBeCulled && !binding.cullPending) {
        try {
          ...
          if (gNode?.gain && ctx?.currentTime !== undefined) {
            ...
            try {
              param.setValueAtTime(previous, t);
            } catch {}
            try {
              param.linearRampToValueAtTime(0, t + dt);
            } catch {}
          }
          if (binding.startedAt !== undefined) {
            ...
            if (binding.committedBuffer) {
              binding.virtualTime =
                binding.virtualTime % binding.committedBuffer.duration;
            }
          }
          const finalize = () => {
            try {
              if ((binding.audio as any).isPlaying) binding.audio.stop();
            } catch {}
            try {
              binding.audio.disconnect();
            } catch {}
            ...
            if (DEBUG_AUDIO) {
              console.log(
                `[Culling] 🔴 CULLED ${nodeId} at distance ${distance.toFixed(
                  2
                )}, saved position: ${binding.virtualTime.toFixed(2)}s`
              );
            }
          };
          ...
          binding.cullPending = false;
          if (DEBUG_AUDIO)
            console.warn(`[Culling] Failed to disconnect ${nodeId}:`, err);
        }
        ...
        if (buffer && binding.pausedAt !== undefined) {
          ...
          try {
            ...
            if (gNode?.gain && ctx?.currentTime !== undefined) {
              ...
              try {
                gNode.gain.linearRampToValueAtTime(target, t + dt);
              } catch {}
            }
            ...
            if (DEBUG_AUDIO) {
              console.log(
                `[Culling] 🟢 RESUMED ${nodeId} at ${newOffset.toFixed(2)}s`
              );
            }
            const delay = Math.max(0, binding.fadeMs || 0);
            if (delay > 0) {
  ...
  getAudioLevel(nodeId: string): number {
    ...
    if (!binding.analyser && (binding.audio as any)?.context) {
      try {
        ...
        if (DEBUG_AUDIO)
          console.warn(`[getAudioLevel] Failed to create analyser:`, err);
        return 0;
      }
    }
  ...
  getSpatialStats(): SpatialStats {
    ...
    for (const [_, binding] of this.spatialBindings) {
      ...
      if (binding.committedBuffer) {
        const bufferSize =
          binding.committedBuffer.length *
          binding.committedBuffer.numberOfChannels *
          4;
  ...
  disposeUnusedBuffers(maxAgeSeconds: number = 60): void {
    ...
    if (DEBUG_AUDIO && disposed > 0) {
      console.log(
        `[lib/audio.ts/disposeUnusedBuffers] Disposed ${disposed} unused buffers`
      );
    }
  }
    ...
    for (const nodeId of this.previewNodes) {
      ...
      if (node) {
        try {
          ...
          if (DEBUG_AUDIO)
            console.warn(`[lib/audio.ts/dispose] Disconnect warning:`, err);
        }
      }
    }
        ...
        if (typeof value === "function") {
          if (prop === "rampTo") {
            return (target: number, seconds: number) => {
              const currentValue =
                node.instance.value !== undefined ? node.instance.value : 0;
  ...
  private applyChange(change: any): void {
    ...
    if (change.type === "param") {
      this.updateParam(change.nodeId, change.param, change.value, {
        record: false,
      });
      ...
      if (change.action === "add") {
        ...
        const index = this.spec.automations.findIndex(
          (a) => a.id === change.id
        );
        if (index !== -1) {
  ...
  setSpatialVolume(volume: number): void {
    ...
    if (DEBUG_AUDIO) {
      console.log(
        `[lib/audio.ts/setSpatialVolume] Spatial volume set to ${clampedVolume}`
      );
    }
  }
  ...
  setLyriaVolume(volume: number): void {
    ...
    if (DEBUG_AUDIO) {
      console.log(
        `[lib/audio.ts/setLyriaVolume] Lyria volume set to ${clampedVolume}`
      );
    }
  }
  ...
  setAmbientVolume(volume: number): void {
    ...
    if (DEBUG_AUDIO) {
      console.log(
        `[lib/audio.ts/setAmbientVolume] Ambient volume set to ${clampedVolume}`
      );
    }
  }
...
export function createSharedAudioContext(
  opts: { sampleRate?: number; latencyHint?: AudioContextLatencyCategory } = {}
): AudioContext {
  ...
  if (DEBUG_AUDIO) {
    console.log(
      `[createSharedAudioContext] Created shared context: ${context.sampleRate}Hz`
    );
  }
...
export async function createAudioSession(
  ...
  if (opts.context) {
    ...
    if (DEBUG_AUDIO) {
      console.log(
        `[createAudioSession] Using provided context, state: ${opts.context.state}`
      );
    }
  } else {
    await Tone.start();
  ...
  if (DEBUG_AUDIO) {
    console.log(
      `[lib/audio.ts/createAudioSession] Created session with context: ${ctx.sampleRate}Hz`
    );
  }
